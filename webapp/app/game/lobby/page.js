...
function LobbyContent() {
  ...
  // Preserves the player's stake when they land here via the cartela-selection
  // page's Back button (which passes ?stake=) instead of always resetting to
  // the first tier.
  const stakeParam = Number(searchParams.get('stake'));
  const [stake, setStake] = useState(STAKES.includes(stakeParam) ? stakeParam : STAKES[0]);
